...
/*
  ...
  server.begin();
*/
  save_request  = "<BODY>N9560980378N9540046952o441o332E</BODY>";//SendWithMessage(host, stream_id);
...
//counting numbers
  for(i=1;i<inf.length();i++){
       if(inf[i]=='o'){arr[special_i]=temp;temp="";special_i=0;break;}
       if(inf[i]=='N'){
        arr[special_i]=temp;
        temp="";
        special_i=special_i+1;}
        else{temp=temp+inf[i];}
    ...
    for(i=1;i<inf.length();i++){
       if(inf[i]=='o'){state=1;}
       if(state==0){continue;}
       if(inf[i]=='E'){otp[special_i]=temp;temp="";special_i++;break;}
       if(inf[i]=='o'){
        otp[special_i]=temp;
        temp="";
        special_i=special_i+1;}
  ...
  //countiong otps
    ...
    Serial.println(otp[1]);
    Serial.println(otp[2]);
